...
@onready var chat_scroll = $TextureRect/Info/Window/Chat/ChatBox.get_v_scroll_bar()
...
func connect_signals():
	chat_scroll.connect("changed", Callable(self, "scroll_chat_box"))
	connect_server_signals()

func connect_server_signals():
	Server.connect("lobby_updated_signal", Callable(self, "lobby_updated"))
	Server.connect("kicked_from_lobby_signal", Callable(self, "kicked_from_lobby"))
	Server.connect("got_message", Callable(self, "got_message"))
	Server.connect("game_started_signal", Callable(self, "start_game"))
...
func instance_player_list():
	...
	for i in Server.my_lobby.players:
		var player_instance = player_instance_scene.instantiate()
		...
		if Server.my_lobby.players[i].id == Server.player_id:
			player_instance.name_label.set("theme_override_colors/font_color", Color.ORANGE_RED)
...
func write_server_message(message):
	var message_instance = message_instance_scene.instantiate()
...
func got_message(message, sender):
	...
	message_instance.sender_label.set("theme_override_colors/font_color", sender.color)
